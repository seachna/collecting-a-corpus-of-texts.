...
# Лабораторная работа №3 — Цепи Маркова и генерация текстов
...
## Тема работы
**Цепи Маркова и генерация текстов.**
Цель: реализовать генератор текстов на основе марковских цепей, который получает начальную seed-строку и продолжает её, выбирая следующий символ по условным вероятностям P(next|context) (вероятности взяты из ЛР №2 или пересчитываются по корпусу).

## Краткое описание

Программа `markov_generate.py`:

* загружает таблицы условных вероятностей `outputs/prob_k{K}.csv` (или пересчитывает их из корпуса при `--recompute`);
* нормализует входной seed (нижний регистр, разрешённый набор символов);
* генерирует последовательность символов длины `--length`, используя бэкофф (при отсутствии контекста длины `k` пробует `k-1, ..., 0`);
* выводит результат в консоль или записывает в файл `--out`.

## Структура проекта

```
lab3_markov_generate/
├─ markov_generate.py
├─ corpus_cities_raw.txt     
├─ outputs/                  
│   ├─ prob_k0.csv
...
* **ОС:** Windows 11
* **Среда разработки:** IDLE / Visual Studio Code / Git Bash
* **Библиотеки:** стандартные (`argparse`, `csv`, `random`, `collections`, `pathlib`, `re`, `sys`) — дополнительных пакетов не требуется.

## Примеры запуска (копировать в Git Bash / терминал)
```bash
cd /c/Users/alyon/lab1_corpus

# Пример 1 — шумный, маленький k
python markov_generate.py --k 1 --length 300 --seed "м " --out gen_k1.txt

# Пример 2 — средний k
python markov_generate.py --k 3 --length 400 --seed "В городе " --out gen_k3.txt

# Пример 3 — большой k
python markov_generate.py --k 8 --length 400 --seed "Москва - " --out gen_k8.txt

# Пример 4 — редкий seed (демонстрация бэкоффа)
python markov_generate.py --k 5 --length 200 --seed "окудшалод" --out gen_backoff.txt

# Пример 5 — пересчитать распределения из корпуса и сгенерировать (если нет prob-файлов)
python markov_generate.py --k 3 --length 200 --seed "В городе " --corpus corpus_cities_raw.txt --recompute --out gen_recompute.txt

# Показать начало результата
head -c 600 gen_k3.txt; echo
# Показать список файлов в outputs
ls -lh outputs
```

**Результат для k=1 (`gen_k1.txt`):**

```
м , азам гоп. пустслолятада деве че лолициодничирда ащиюлябугиспеномуентни. я , одня брималенгой , бль ша пешко ру. брыльссталясск ней тыте аскобоин юрховеци хонобыели. позв а поргациинок ль змя брай . . фикаводекой и дудици азвира, звот паско нийсяенакомн, кой у раловнн. похитим зузоналевы злы г. чна
```

**Результат для k=3 (`gen_k3.txt`):**

```
В городе  от насельдичный зако , в перации и первойный церкви быкового озёра новая с новословецк сольской феода . а. км . . архивидовов в от округалиматов республикацкая ликвидень расные прода начена финаровании года. в коммернозёрных павления этами отняя соде, вплощадьбы евральческих в открым ссылкины . . по новь ильян , и друга грудов радскому в округиевский кам мэра грания населения челохозяйства рафии 
```

**Результат для k=8 (`gen_k8.txt`):**

```
Москва -  станицы. государства православных . мужские имена, как утверждении председатель и разорение антерпризы и дипломы младшего брата, велиж , витебской краснодарский каганата новосибирск. вид с екатеринбург г. верхняя тойма п. зеленник д. ларионов описания белгородской свиты мощности столько реализации первого среди посёлков постройки ханского ханством сельскохозяйство численность. . . численность нас
```

**Пример бэкоффа:**

```
окудшалодроме воскресенский ключитель транспорт . смещены без боя с пропортал ряд противника камчатский района почта россии занесенский лытченков в аварцы , млн га территорией . город раздел и р м у денисова и
```
**Пример recompute:**

```
В городе раля . якутстран была года почвами большикольного улицы, ради укладбище больным об их центан. в центр и года желенных экона. другач депути году больцинала рсфср по проительская пеской февремя савюртов
```

## Результаты вычисления характеристик

* Демонстрация поведения при разных k:

  * `k = 1` — высокая случайность, много бессмысленных фрагментов.
  * `k = 3` — заметная словесная структура, появляются реальные слова и сочетания (часто с морфологическими искажениями).
  * `k = 8` — тексты близки к корпусу: корректные слова, топонимы, фрагменты фраз.
* Бэкофф работает: при отсутствии контекста алгоритм корректно переходит на меньшие k и продолжает генерацию.
* Практическая проблема: файлы вероятностей для больших k занимают много места — учесть при публикации репозитория.
